...
public class CmsTokenSigningService {
    ...
    private static final String DSC_PIN = "123456789"; // Your DSC PIN
    ...
    public void init() {
        try {
            ...
            // Load Windows-MY keystore (where USB token appears)
            loadKeyStore();
    ...
    private void loadKeyStore() throws Exception {
        ...
        keyStore.load(null, DSC_PIN.toCharArray());
        logger.info("✅ KeyStore loaded successfully: Windows-MY");

        // Get SunMSCAPI provider for signing
        signingProvider = Security.getProvider("SunMSCAPI");
        if (signingProvider == null) {
            throw new Exception("SunMSCAPI provider not available");
        }
        logger.info("✅ Signing provider: SunMSCAPI");
        
        logger.info("✅ DSC PIN configured");
    }
    ...
    public SigningResult signData(byte[] dataBytes) {
        try {
            ...
            PrivateKey privateKey = (PrivateKey) keyStore.getKey(alias, DSC_PIN.toCharArray());
            ...
            // CRITICAL: Use SunMSCAPI provider for signing (not BouncyCastle)
            ContentSigner contentSigner = new JcaContentSignerBuilder("SHA256withRSA")
                    .setProvider(signingProvider)  // Use SunMSCAPI for MSCAPI keys
                    .build(privateKey);
