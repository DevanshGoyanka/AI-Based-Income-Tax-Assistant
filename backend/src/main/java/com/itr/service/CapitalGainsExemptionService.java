...
public class CapitalGainsExemptionService {
     ...
     * - Exemption = lower of (capital gain, investment)
     */
    public Exemption54BResult calculate54BExemption(double capitalGain, double investmentAmount, 
                                                      LocalDate saleDate, LocalDate purchaseDate) {
        Exemption54BResult result = new Exemption54BResult();
        result.setCapitalGain(capitalGain);
        result.setInvestmentAmount(investmentAmount);
        
        // Validate time period: 2 years from sale date
        if (purchaseDate != null && saleDate != null) {
            long daysDiff = ChronoUnit.DAYS.between(saleDate, purchaseDate);
            if (daysDiff < 0 || daysDiff > 730) { // 2 years = 730 days
                result.setEligible(false);
                result.setReason("Agricultural land must be purchased within 2 years of sale");
                result.setExemptionAmount(0);
                result.setTaxableGain(capitalGain);
            } else {
                result.setEligible(true);
                double exemption = Math.min(capitalGain, investmentAmount);
                result.setExemptionAmount(exemption);
            ...
            result.setEligible(false);
            result.setReason("Sale and purchase dates required for 54B exemption");
            result.setExemptionAmount(0);
            result.setTaxableGain(capitalGain);
        }
     ...
     * - Exemption = lower of (capital gain, investment)
     */
    public Exemption54DResult calculate54DExemption(double capitalGain, double investmentAmount,
                                                      LocalDate saleDate, LocalDate purchaseDate) {
        Exemption54DResult result = new Exemption54DResult();
        result.setCapitalGain(capitalGain);
        result.setInvestmentAmount(investmentAmount);
        
        // Validate time period: 3 years from sale date
        if (purchaseDate != null && saleDate != null) {
            long daysDiff = ChronoUnit.DAYS.between(saleDate, purchaseDate);
            if (daysDiff < 0 || daysDiff > 1095) { // 3 years = 1095 days
                result.setEligible(false);
                result.setReason("Replacement land/building must be purchased within 3 years of compulsory acquisition");
                result.setExemptionAmount(0);
                result.setTaxableGain(capitalGain);
            } else {
                result.setEligible(true);
            ...
            result.setEligible(false);
            result.setReason("Acquisition and purchase dates required for 54D exemption");
            result.setExemptionAmount(0);
            result.setTaxableGain(capitalGain);
        }
     ...
     * - Exemption = lower of (capital gain, investment)
     */
    public Exemption54GResult calculate54GExemption(double capitalGain, double investmentAmount,
                                                     LocalDate saleDate, LocalDate purchaseDate) {
        Exemption54GResult result = new Exemption54GResult();
        result.setCapitalGain(capitalGain);
        result.setInvestmentAmount(investmentAmount);
        ...
        if (purchaseDate != null && saleDate != null) {
            long daysDiff = ChronoUnit.DAYS.between(saleDate, purchaseDate);
            if (daysDiff < -365 || daysDiff > 1095) { // 1 year before to 3 years after
                result.setEligible(false);
                result.setReason("New assets must be purchased within 1 year before or 3 years after shifting");
                result.setExemptionAmount(0);
                result.setTaxableGain(capitalGain);
            } else {
                result.setEligible(true);
            ...
            result.setEligible(false);
            result.setReason("Transfer and purchase dates required for 54G exemption");
            result.setExemptionAmount(0);
            result.setTaxableGain(capitalGain);
        }
     ...
     */
    public Exemption54GBResult calculate54GBExemption(double capitalGain, double investmentAmount,
                                                       LocalDate saleDate, LocalDate investmentDate) {
        Exemption54GBResult result = new Exemption54GBResult();
        result.setCapitalGain(capitalGain);
        result.setInvestmentAmount(investmentAmount);
        
        // Validate time period: 6 months from sale date
        if (investmentDate != null && saleDate != null) {
            long daysDiff = ChronoUnit.DAYS.between(saleDate, investmentDate);
            if (daysDiff < 0 || daysDiff > 180) {
                result.setEligible(false);
                result.setReason("Startup investment must be made within 6 months of property sale");
                result.setExemptionAmount(0);
                result.setTaxableGain(capitalGain);
            } else {
                // Cap at ₹50 lakhs
                ...
                result.setExemptionAmount(exemption);
                result.setTaxableGain(capitalGain - exemption);
                result.setLockInYears(5);
            }
        } else {
            result.setEligible(false);
            result.setReason("Sale and investment dates required for 54GB exemption");
            result.setExemptionAmount(0);
            result.setTaxableGain(capitalGain);
        }
    ...
    @Data
    public static class Exemption54BResult {
        private double capitalGain;
        private double investmentAmount;
        private boolean eligible;
        private String reason;
    ...
    @Data
    public static class Exemption54DResult {
        private double capitalGain;
        private double investmentAmount;
        private boolean eligible;
        private String reason;
    ...
    @Data
    public static class Exemption54GResult {
        private double capitalGain;
        private double investmentAmount;
        private boolean eligible;
        private String reason;
    ...
    @Data
    public static class Exemption54GBResult {
        private double capitalGain;
        private double investmentAmount;
        private boolean eligible;
        private String reason;
    ...
    @Data
    public static class Exemption54EEResult {
        private double capitalGain;
        private double investmentAmount;
        private boolean eligible;
        private String reason;
    ...
    /**
     * Section 54EE - Exemption on transfer of original asset for investment in startup
     * (Inserted by Finance Act 2017, withdrew by Finance Act 2023 - Now not applicable from AY 2024-25)
     * Kept for archival purposes and old ITR filing
     * 
     * Conditions:
     * - Investment in units of eligible startup within 6 months
     * - Lock-in: 5 years
     * - Max exemption: ₹50,00,000 per assessee
     */
    public Exemption54EEResult calculate54EEExemption(double capitalGain, double investmentAmount,
                                                      LocalDate saleDate, LocalDate investmentDate) {
        Exemption54EEResult result = new Exemption54EEResult();
        result.setCapitalGain(capitalGain);
        result.setInvestmentAmount(investmentAmount);
        
        // Section 54EE was withdrawn from AY 2024-25 (Budget 2023)
        result.setEligible(false);
        result.setReason("Section 54EE is not applicable from AY 2024-25 as per Finance Act 2023");
        result.setExemptionAmount(0);
        result.setTaxableGain(capitalGain);
