...
public class ClientService {
    ...
    @Transactional
    public void updateItrType(Long clientId, String year, String itrType, Long userId) {
        Client client = clientRepository.findByIdAndUserId(clientId, userId)
                .orElseThrow(() -> new ResourceNotFoundException("Client not found"));
        
        ClientYearData yearData = client.getYearData().stream()
                .filter(yd -> yd.getAssessmentYear().equals(year))
                .findFirst()
                .orElseThrow(() -> new ResourceNotFoundException("Year data not found"));
        
        // Validate ITR type
        if (!itrType.matches("ITR[1-7]")) {
            throw new IllegalArgumentException("Invalid ITR type. Must be ITR1-ITR7");
        }
        
        yearData.setItrType(itrType);
