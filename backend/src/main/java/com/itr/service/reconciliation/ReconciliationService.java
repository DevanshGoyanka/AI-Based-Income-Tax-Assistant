...
public class ReconciliationService {
    ...
    private Map<String, TDSEntry> build26ASMap(Form26ASData f26as) {
        ...
        // Process all TDS entries from Form26ASData using new structure
        if (f26as != null) {
            // TDS on Interest
            for (Form26ASData.TDSOtherThanSalary entry : f26as.getTdsOnInterest()) {
                addToMap(map, entry.getDeductorTAN(), entry.getSection(), entry.getDeductorName(), entry.getTaxDeducted());
            }
            // TDS on Contractors
    ...
    private void addToMap(Map<String, TDSEntry> map, String tan, String section, String name, java.math.BigDecimal tax) {
        if (tan == null || tan.isEmpty()) return;
        String key = tan + "_" + (section != null ? section : "OTHER");
        map.put(key, new TDSEntry(
            name != null ? name : "",
            tan,
            section != null ? section : "OTHER",
            tax != null ? tax.doubleValue() : 0
        ));
    }
